...
def run(args):
    ...
    # This uses the unique_mapper_bam filename given as an argument to run() to extract
    # reads that map to repetitive regions, using coverageBed and the repnames.bed file
    # that holds genomic locations.
    # head hg38re/repnames.bed gives this format:
    # chr1    10469   11447   TAR1
    # chr1    11505   11675   L1MC5a
    # The output of coverageBed is sent to a *_regionsorter.txt file, which has this format:
    # chr1   10469   11447   TAR1    0   0   978 0.0000000
    # chr1    11505   11675   L1MC5a  0   0   170 0.0000000

    print('Conducting region sorting on unique mapping reads....')
    def run_coverage_bed_on_uniquely_bammed_bams():
        ...
        p.communicate()
        stdout.close()
    run_coverage_bed_on_uniquely_bammed_bams()

    # Put the output of the above into a dict (counts) of repeatname -> number of reads.
    filein = open(outputfolder + os.path.sep + outputfile_prefix + '_regionsorter.txt', 'r')
    #filein = open('coverageBed_applied_to_empty_bam_file_and_Alu_repnames.bed', 'r')
    counts = {}
    sumofrepeatreads = 0
    ...
    ################################################################################
        ...
        for n_genome, metagenome in enumerate(repeat_list):
            ...
            # -I gives the mimimum insert length.
